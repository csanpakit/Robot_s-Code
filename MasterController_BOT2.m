...
configurePin(mypi,22,'PWM')

%GPIO for Left Motor GPIO 23 and 24
configurePin(mypi,24,'PWM')
...
while 1
    ...
    [distance_traveled_turn, theta_current_turn] = turn2(desired_angle + 180,theta_current,mypi,m,cam);
    pause(2) %add pause for stability
    %%%%%%%%%%%%%%%%drive_straight until destination reached%%%%%%%%%%%%%%%%%%%
    
    [distance_current, theta_current_drive] = drive_straight2(mypi,distance_to_travel,m,cam);
    pause(2)
    %%%%%%%%%%%%%%%%%%pass in correct orientation%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %theta_current_turn will return how much the robot turned, but not
    %necessarily which direction
    theta_current_turn;
    theta_current_drive;
...
%%%%%%%%%%%%%%%%%%%%%%%%%%DRAW ROBOT POSE%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%    
    ...
    plot(x_current,y_current,'b*','LineWidth',2)
    %drawnow;
    %delete(h2)
...
%%%%%%%%%%%%%%%%%%%%%%%%%%RUN RRT%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [x_new y_new] = RRT_Drive2(x_current,y_current,theta_current,all_samples);
...
writePWMDutyCycle(mypi, 22, 0);
writePWMDutyCycle(mypi, 24, 0);
